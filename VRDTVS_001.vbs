...
Function vrdtvs_Convert_File (	byVal	CF_FILE_AbsolutePathName, _
	...
	Dim ff_cmd_string, ff_tmp_object, ff_logfile, ff_batfile, ff_cmd_string_for_bat, ff_run_errorlevel
	'
	...
	ff_cmd_string_for_bat = ff_cmd_string & " >""" & ff_logfile & """ 2>&1"
	...
	Wscript.Echo "About to .Run: " & ff_batfile
	ff_run_errorlevel = wso.Run("CMD /C """ & ff_batfile & """", 7, True) '(strCommand, [intWindowStyle], [bWaitOnReturn]) ' https://ss64.com/vb/run.html
	Wscript.Echo ".Run exit vrdtvs_status=" & ff_run_errorlevel
	vrdtvs_status = vrdtvs_delete_a_file (ff_batfile, False)		' Delete the .bat file to be created with the ffmpeg command
	' ff_run_errorlevel now contains the errorlevel returne by ffmpeg 
	' let's dump the ffmpeg log
