...
IF /I "!EL!" NEQ "0" (
	...
	ECHO !DATE! !TIME! >> "!vrdlog!" 2>&1
	ECHO !DATE! !TIME! *********  QSF Error QSF failed on "%~f1", QSF file not created "!QSF_File!" >> "!vrdlog!" 2>&1
	ECHO !DATE! !TIME! >> "!vrdlog!" 2>&1
	set "check_qsf_failed=True"
) ELSE if NOT exist "!temp_cmd_file!" ( 
	ECHO !DATE! !TIME! >> "!vrdlog!" 2>&1
	ECHO !DATE! !TIME! *********  QSF Error Temp cmd file "!temp_cmd_file!" not generated by QSF of "%~f1" >> "!vrdlog!" 2>&1
	ECHO !DATE! !TIME! >> "!vrdlog!" 2>&1
	set "check_qsf_failed=True"
)
...
echo "!py_exe!" !Path_to_py_VRDTVSP_Calculate_Duration! --start_datetime "!gather_variables_from_media_file_START!" --end_datetime "!gather_variables_from_media_file_END!" --prefix_id "gather !current_prefix!" >> "!vrdlog!" 2>&1
